...
def move_piece(event, row=None, col=None):
    ...
    if selected is None:
        ...
        if (row, col) != selected and (row, col) not in moves and (row, col, 'promotion') not in moves and \
                (row, col, 'en passant') not in moves and (row, col, 'castling') not in moves:
            return
        selected = None
        ...
        if from_row != row or from_col != col:
            ...
            board.set_prev_board(history[-1])
            board.move_piece(from_row, from_col, row, col)
            if row == 0 and board.data[row][col] == 'wp':
                board.data[row][col] = 'wq'
            elif board.data[row][col] == 'wp' and history[-1].data[row][col] == '.':
                board.data[row + 1][col] = '.'
            elif board.data[row][col].endswith('k') and abs(col - from_col) == 2:
                if col == board.cols - 2:
                    board.move_piece(from_row, board.cols - 1, row, board.cols - 3)
                elif col == 2:
                    board.move_piece(from_row, 0, row, 3)
...
def update_board(row, col):
    ...
    if selected is not None:
        color = 'salmon'
        if (row, col) in moves or (row, col, 'promotion') in moves or (row, col, 'en passant') in moves or (row, col, 'castling') in moves:
            color = 'light green'
    else:
